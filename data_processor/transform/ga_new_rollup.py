...
class NetworkGraphTransformer(Transformer):
    ...
    def __init__(self, lvl_thresholds=None, global_threshold=0.1,
                 last_viable_lvl=10, rollup_start_lvl=5, rollup_end_lvl=2):
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['previousPage'] = df['previousPage'].apply(lambda x: re.sub("/[\d]+", "/:id", x))

        logging.info('{msg}: {x}'.format(msg='No. of unique URLs after regex ', x=df['pagePath'].nunique()))
            ...
            if page in rolled_urls:
                ...
                for rolled_url in rolled_urls:
                    if rolled_url in page:
                        if (page[:len(rolled_url)] == rolled_url) and (len(rolled_url) > x):
                            x = len(rolled_url)
    ...
    def create_lvls(self, df: pd.DataFrame) -> pd.DataFrame:
        df['lvl0'] = '/'
        for i in range(1, 10):
            df['lvl{}'.format(i)] = df['pagePath'].apply(lambda x: NetworkGraphTransformer.augment(x, i))
    ...
    def levelwise_rollup(self, data, lvl, threshold):
        ...
        logging.info('{msg}: {x}'.format(msg='Total PageViews Count before rollup', x=data['pageViews'].sum()))

        curr_lvl_col = 'lvl{}'.format(lvl)
        for i in range(1, lvl + 1):
            data['lvl{}'.format(i)] = data['rolled_url'].apply(lambda x: NetworkGraphTransformer.augment(x, i))
        ...
        for i, idf in data1.groupby(by=prev_lvls):
            ...
            x2 = idf[(idf['pct_pageviews'] > threshold) & (idf['pageViews'] > self.global_threshold)][
        ...
        res_df = res_df.groupby('rolled_url', sort=False, as_index=False).agg(lambda x: x.sum())

        logging.info('{msg}: {x}'.format(msg='Total PageViews Count after rollup', x=res_df['pageViews'].sum()))
    ...
    def generate_rolled_urls(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.last_viable_lvl > self.rollup_start_lvl:
            logging.info('{msg}: {x}'.format(msg='Automatic rollup for higher lvls', x='starts'))
