...
async def get_edges(ds_id: str, start_date: str = '1970-01-01', end_date: str = dt.today().strftime('%Y-%m-%d'),
...
async def get_trend_nodes(ds_id: str, node: str, trend_type: str,
                          start_date: str = '1970-01-01', end_date: str = dt.today().strftime('%Y-%m-%d'),
                          edge_service: EdgeService = Depends()):
    return await edge_service.get_node_trends(ds_id, node, trend_type, start_date, end_date)


@router.get("/datasources/{ds_id}/sankey", response_model=list[NodeSankeyResponse])
async def get_sankey_nodes(ds_id: str, node: str,
                           start_date: str = '1970-01-01', end_date: str = dt.today().strftime('%Y-%m-%d'),
                           edge_service: EdgeService = Depends()):
