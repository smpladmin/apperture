...
class TestRoleRepository:
    def setup_method(self):
        ...
        self.role = Role(clickhouse=self.clickhouse)
        ...
        self.grant_select_permission_query=f"GRANT SELECT ON events TO {self.username}"
    ...
    def test_create_user(self):
        self.role.create_user(username=self.username, password=self.password)
        self.clickhouse.admin.query.assert_called_with(query=self.create_user_query)

    def test_grant_select_permission_to_user(self):
        self.role.grant_select_permission_to_user(username=self.username)
        self.clickhouse.admin.query.assert_called_with(query=self.grant_select_permission_query)

    def test_create_row_policy(self):
        self.role.create_row_policy(datasource_id=self.dsId, username=self.username)
        self.clickhouse.admin.query.assert_called_with(query=self.create_row_policy_query)
