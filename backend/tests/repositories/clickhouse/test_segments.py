...
class TestSegmentsRepository:
        ...
        calls = [
            call(
                ...
                query=(
                    ...
                    '"user_id" FROM "events" WHERE "datasource_id"=%(ds_id)s AND '
                    "\"properties.prop1\" IN ('va1','val2') AND \"properties.prop2\" IN "
                    "('va3','val4')) AND \"event_name\"='Topic_Click' GROUP BY "
                    '"user_id" HAVING COUNT("user_id")=\'2\') ,cte3 AS (SELECT "user_id" '
                    'FROM "events" WHERE "datasource_id"=%(ds_id)s AND "user_id" IN '
                    '(SELECT DISTINCT "user_id" FROM "events" WHERE '
                    '"datasource_id"=%(ds_id)s AND "properties.prop1" IN '
                    "('va1','val2') AND \"properties.prop2\" IN ('va3','val4')) "
                    'AND "event_name"<>\'Video_Open\') SELECT DISTINCT "cte2"."user_id" '
                    'FROM cte2 UNION ALL SELECT DISTINCT "cte3"."user_id" FROM cte3) '
                    'SELECT DISTINCT "group0"."user_id" FROM group0 INTERSECT SELECT '
                    'DISTINCT "group1"."user_id" FROM group1 INTERSECT SELECT DISTINCT '
    ...
    def test_build_who_clause_users_query_for_who_filters_with_date(self):
        segment_users = self.repo.get_all_unique_users_query()
        segment_users = self.repo.build_who_clause_users_query(
            group=SegmentGroup(
                filters=self.who_filters_with_dates,
                condition=SegmentGroupConditions.AND,
            ),
            group_users=segment_users,
            idx=0,
        )
        assert segment_users.get_sql() == (
            'WITH cte0 AS (SELECT "user_id" FROM "events" WHERE "datasource_id"=%(ds_id)s '
            'AND "user_id" IN (SELECT DISTINCT "user_id" FROM "events" WHERE '
