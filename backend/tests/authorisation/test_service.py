...
class TestAuthService:
    ...
    def test_verify_password(self):
        ...
        assert not self.service.verify_password(
            self.hashed_password, self.wrong_password
        )

    def test_rehash_password(self):
        assert "argon" in self.service.rehash_password(
            hash=self.hashed_password, password=self.password
        )
